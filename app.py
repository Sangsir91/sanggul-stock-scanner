...
st.set_page_config(page_title='Sanggul Stock Scanner V8.1', page_icon='📈', layout='wide')
...
<style>
    ...
    .section-note {
        ...
        padding:10px 14px; border-radius:8px; color:#475569;
        margin: 4px 0 14px 0;
    }
    .score-note {
...
def bounded_score(value, center=0.0, scale=10.0):
    """Convert a percentage/ratio into a smooth 0-100 score without saturation."""
    try:
        z = (float(value) - center) / max(float(scale), 1e-9)
        return float(np.clip(50.0 + 50.0 * np.tanh(z), 0, 100))
    except Exception:
        return 50.0

def quality_score(value, low=0.0, high=2.0):
    try:
...
def analyze_period(df, focus_days=63):
    ...
    # Continuous scoring: avoid the old binary 0/20 and 0/25 jumps
    # that caused many stocks to receive exactly 100.
    ma20_gap = (px / ma20 - 1) * 100 if ma20 else 0
    ma50_gap = (px / ma50 - 1) * 100 if ma50 else 0
    ma200_gap = (px / ma200 - 1) * 100 if ma200 else 0
    alignment = 100 if ma20 > ma50 > ma200 else bounded_score(
        ((ma20/ma50-1)*100 if ma50 else 0) + ((ma50/ma200-1)*100 if ma200 else 0),
        center=0, scale=8
    )
    trend_score = (
        0.22 * bounded_score(ma20_gap, 0, 4) +
        0.22 * bounded_score(ma50_gap, 0, 6) +
        0.22 * bounded_score(ma200_gap, 0, 10) +
        0.19 * alignment +
        0.15 * bounded_score(ret_focus, 0, 8)
    )

    ret20 = safe_num(latest['Return20D'], 0)
    ret60 = safe_num(latest['Return60D'], 0)
    rsi_quality = 100 - min(abs(rsi - 58) * 2.2, 100)
    macd_gap = ((macd - macds) / max(abs(macds), 0.01)) * 100
    volume_quality = bounded_score((vr - 1) * 100, 0, 35)
    momentum_score = (
        0.28 * bounded_score(ret20, 0, 8) +
        0.24 * bounded_score(ret60, 0, 15) +
        0.20 * rsi_quality +
        0.16 * bounded_score(macd_gap, 0, 35) +
        0.12 * volume_quality
    )
...
def enrich_top150(base_df, focus_days, progress_callback=None):
    ...
    for idx, (_, r) in enumerate(target.iterrows(), start=1):
        ...
        try:
            ...
            pass
        # Style scores use normalized continuous components.
        # IMPORTANT: do not add +50 here because each component is already 0-100.
        # Adding +50 caused nearly every score to saturate at 100.
        day_momentum = bounded_score(a['Return5D'], 0, 4)
        day_volume = bounded_score((a['VolumeRatio'] - 1) * 100, 0, 45)
        day_setup = 90 if a['Breakout'] == 'YA' else 62 if a['Setup'] == 'PULLBACK' else 42
        style_day = 0.42*day_momentum + 0.33*day_volume + 0.25*day_setup

        swing_20 = bounded_score(a['Return20D'], 0, 8)
        swing_60 = bounded_score(a['Return60D'], 0, 15)
        swing_setup = 88 if a['Setup'] == 'PULLBACK' else 78 if a['Setup'] == 'BREAKOUT' else 48
        style_swing = 0.30*swing_20 + 0.25*swing_60 + 0.30*a['TechnicalScore'] + 0.15*swing_setup

        fundamental_score = 50.0
        if not pd.isna(roe): fundamental_score += np.clip(roe, -20, 30) * 0.8
        if not pd.isna(pe) and pe > 0: fundamental_score += 12 if pe < 15 else 6 if pe < 25 else -4
        if not pd.isna(pbv) and pbv > 0: fundamental_score += 10 if pbv < 2 else 5 if pbv < 4 else -3
        fundamental_score = float(np.clip(fundamental_score, 0, 100))
        investor = 0.55*fundamental_score + 0.25*a['TechnicalScore'] + 0.20*bounded_score(a['Return60D'], 0, 15)
        row={**r.to_dict(), **a,
             'PER':pe,'PBV':pbv,'ROE_pct':roe,'MarketCap':market_cap,
             'DayScore':round(float(np.clip(style_day,0,100)),1),
             'SwingScore':round(float(np.clip(style_swing,0,100)),1),
...
st.caption('Multi-horizon dengan scoring kontinu V8.2: skor gaya dihitung langsung pada skala 0–100 tanpa bonus tersembunyi yang membuat semua nilai menjadi 100.')
...
st.markdown(
    '<div class="score-note"><b>Catatan skor:</b> skor sekarang menggunakan perhitungan kontinu '
    '(momentum, jarak terhadap MA, volume, setup, dan risiko), bukan lagi sekadar penjumlahan sinyal biner. '
    'Karena itu nilai 100 hanya mungkin untuk kondisi yang benar-benar sangat kuat, bukan otomatis.</div>',
    unsafe_allow_html=True
)
...
if menu=='🌐 Full IDX Scanner':
    ...
    if not result.empty:
        ...
        st.caption('Peringkat awal berdasarkan kombinasi Technical Score, Opportunity, momentum, volume, dan risk/reward.')
        ...
        if not enriched.empty:
            ...
            st.caption('Semua skor di bawah dihitung ulang dari data 2 tahun dengan fokus periode yang dipilih. DayScore dan SwingScore berada pada skala 0–100 tanpa penambahan +50.')
