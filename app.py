...
[data-testid="stMetric"] {background: linear-gradient(135deg,#f8fbff,#eef5ff); border:1px solid #dce8f7; padding:14px 16px; border-radius:14px; box-shadow:0 4px 14px rgba(30,70,120,.06);}
[data-testid="stDataFrame"] {border:1px solid #dce4ef; border-radius:14px; overflow:hidden;}
...
def fast_analysis(df):
    ...
    # V8.2: continuous scoring, agar skor tidak menumpuk menjadi 100
    # Setiap komponen dinormalisasi 0-100 lalu diberi bobot.
    trend_px = np.clip(((px / x["MA20"]) - 0.97) / 0.10 * 100, 0, 100)
    trend_ma = np.clip(((x["MA20"] / x["MA50"]) - 0.97) / 0.08 * 100, 0, 100)
    long_trend = np.clip(((px / x["MA200"]) - 0.90) / 0.25 * 100, 0, 100)
    rsi_quality = np.clip(100 - abs(float(x["RSI"]) - 58) * 2.2, 0, 100)
    macd_gap = (float(x["MACD"]) - float(x["MACDSignal"])) / max(abs(px), 1e-9)
    macd_quality = np.clip(50 + macd_gap * 5000, 0, 100)
    volume_quality = np.clip((float(x["VolumeRatio"]) - 0.60) / 2.40 * 100, 0, 100)
    structure = np.clip(((px / max(float(x["Support"]), 1e-9)) - 1.0) / 0.30 * 100, 0, 100)
